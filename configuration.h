...
const int condLength = 512; //256; //4626; //for review analysis 300, 500, 1000, 1500, 2000, 2257
//const int posBits = 2; //2, 3, 4, 5, 6, and 7 for 6-, 11-, 20-, 37, 70-, and 135-bits MUX respectively
const int maxPopSize=  1 * totalNumInstances; //Specifies the maximal number of micro-classifiers in the population. [ (0.5, 1, 2, 5, 10/20, 50)*1000 for 6-, 11-, 20-, 37-, 70-, 135-bits MUX respectively]
...
const int maxProblems = 50 * totalNumInstances; //1*100*1000; //training set = [ (1, 1.5, 2, 2.5, 3,)*100*1000 for 6-, 11-, 20-, 37-, 70-, 135-bits MUX respectively]
const double maxPayoff = 1000;
const int clfrCondLength = 64; //32; //300;//condLength/4; // condLength/2 and condLength/4 for 70mux and 135mux respectively.
const int run = 1;
const char inputFile[] = "features1.txt";
//const char testFile[] = "";
const bool Testing = true;
const int currentProblemLevel = 2; //must be set coz previous level and current level file names are adjusted
